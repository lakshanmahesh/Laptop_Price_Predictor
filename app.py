...
@app.route('/',methods=['POST','GET']) #addressing the root URL example: http://localhost:5000/
def index():
    # return "Hello World"
    pred = 0
    if request.method == 'POST':
        ...
        pred = np.round(pred[0],2)*1.16
    return render_template('index.html',pred = np.round(pred,2))
